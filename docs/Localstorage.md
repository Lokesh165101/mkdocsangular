local storage and custom vlidation 
```
-----------------------Local storage with crud and displaying in the table  ----------------------------
  ...
  <div class="body">
    <!-- <div class="form-group">
      ...
      <input type="text" placeholder="name" id="name" class="form-control" formControlName="name" (blur)="test()">

      <div *ngIf="form.get('name')?.touched && form.get('name')?.errors">
        plaese enter full name
      </div>
      <div *ngIf="form.get('name')?.touched && form.get('name')?.hasError('noplace')">
    ...
    <div class="form-group">
      ...
      <div formArrayName="item">
        ...
        <button type="button" (click)="add()" [disabled]="!(this.item.value[this.item.length-1].hobbies)" >ADDME+</button>
...
@Component({
  ...
  standalone: true,
  imports: [NgIf, MatRadioModule, MatButtonModule, NgFor, MatFormFieldModule, MatSelectModule, MatInputModule,ReactiveFormsModule,FormsModule],
 ...
 return this.form.get('name')?.value
  ...
  add() {
    ...
    for (let i = 0; i <= 60; i++) {
      ...
      this.arrayage.push(i)

    }
    debugger
    this.item.push(this.ok())
  }
  ...
  deletee(i:number){
     ...
     localStorage.setItem("completedata", JSON.stringify(this.array))
  }
...
// function createPasswordStrengthValidator(): ValidatorFn | null {
